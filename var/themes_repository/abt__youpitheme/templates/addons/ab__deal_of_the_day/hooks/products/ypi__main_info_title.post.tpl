		...
		<div class="{if $promotion.to_date && $promotion.to_date > $smarty.now}col1{/if}">
	        ...
	        <div class="actions-link">
	            ...
	            {if $promotion.status == 'A'}
		        	<a class="details-promo-link" href="{"promotions.view?promotion_id=`$promotion.promotion_id`"|fn_url}" title="" target="_blank"><i class="cm-tooltip ty-icon-help-circle" title="{$promotion.short_description}"></i><span>{__('ab__dotd.detailed')}</span></a>
	            {/if}
			</div>
        </div>
