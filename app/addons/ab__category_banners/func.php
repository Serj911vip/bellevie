...
foreach (glob(Registry::get("config.dir.addons") . "/ab__category_banners/functions/*.php") as $functions) require_once $functions;
...
Registry::set('ab__cb_active_banner', $banner);
}
}
return $banner;
}
...
if ($category_banner['position'] <= $products_nodes->length) {
...
$target_node = $products_nodes->item($category_banner['position'] - 1);
$target_node->parentNode->insertBefore($doc->importNode($banner_node->documentElement, true), $target_node);
if (!empty($match[1])) {
$parent_node = $target_node->parentNode;
$empty_items = $match[1] - (($products_nodes->length + 1) % $match[1]);
...
{
$banner = Registry::get('ab__cb_active_banner');
return empty($banner) ? '' : $banner['position'] . '.' . $banner['category_banner_id'];
}