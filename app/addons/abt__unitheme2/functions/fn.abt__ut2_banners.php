...
if (!empty($banner) and in_array($banner['type'], array(ABT__UT2_BANNER_TYPE))){
...
and !empty($devices_enabled_fields[$device])
and $banner["abt__ut2_{$device}_use"] == 'Y'
){
...
$table = '?:banners';
if (in_array($f, array('button_text', 'title', 'url', 'description', 'youtube_id'))){
...
if (!empty($d['abt__ut2_youtube_autoplay']) and $d['abt__ut2_youtube_autoplay'] == 'Y'){
$params['autoplay'] = "autoplay=1";
$params['mute'] = "mute=1";
}
if (!empty($d['abt__ut2_youtube_hide_controls']) and $d['abt__ut2_youtube_hide_controls'] == 'Y'){
