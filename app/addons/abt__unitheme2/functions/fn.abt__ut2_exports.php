...
'abt__ut2_main_image' => array(
'type' => 'M',
'device' => 'all'
),
'abt__ut2_background_image' => array(
'type' => 'A',
'device' => 'all'
),
'abt__ut2_tablet_main_image' => array(
'type' => 'M',
'device' => 'tablet'
),
'abt__ut2_tablet_background_image' => array(
'type' => 'A',
'device' => 'tablet'
),
'abt__ut2_mobile_main_image' => array(
'type' => 'M',
'device' => 'mobile'
),
'abt__ut2_mobile_background_image' => array(
'type' => 'A',
'device' => 'mobile'
)
);
...
$temporal['images'][$new_type] = array( 'img' => $name, 'type' => $type['type'], 'device' => $type['device']);
}
...
fn_set_notification('N', __('notice'), __('abt__ut2.export.success.banners', array('[ids]' => substr($notif, 0, -1))));
}
function fn_abt__ut2_export_blog( ) {
$path = ABT__UT2_DATA_EXP_PATH . 'blog/';
fn_rm($path);
fn_mkdir($path);
$blog_pages = array_shift(fn_get_pages(array(
'page_type' => 'B',
'get_tree' => 'multilevel'
), 0, 'en')[0])['subpages'];
...
fn_set_notification('N', __('notice'), __('abt__ut2.export.success.menu', array('[ids]' => substr($notif, 0, -1))));
}
...
foreach ( $arr as $unset ) { unset($banner[$unset]); }
}
...
foreach ( $arr as $unset ) { if( isset($page[$unset]) ) { unset($page[$unset]); } }
}
