...
$products_cids = db_get_fields('SELECT GROUP_CONCAT(pc.category_id) AS categories FROM ?:products AS products
LEFT JOIN ?:products_categories AS pc ON products.product_id = pc.product_id ?p
WHERE ?p GROUP BY products.product_id', implode(' ', $join), $where);
$cids = array();
...
$params['ab__dotd_join'] = implode(' ', $join);
