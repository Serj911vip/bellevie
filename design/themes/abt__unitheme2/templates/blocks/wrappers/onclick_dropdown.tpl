{if $content|trim}
    ...
    <div class="ty-dropdown-box {if $block.user_class} {$block.user_class}{/if}{if $content_alignment == "RIGHT"} ty-float-right{elseif $content_alignment == "LEFT"} ty-float-left{/if}">
        <div id="sw_dropdown_{$dropdown_id}" class="ty-dropdown-box__title {if $runtime.controller != 'index'}cm-combination {else} open {/if} {if $header_class}{$header_class}{/if}">
        ...
        <div id="dropdown_{$dropdown_id}" class="cm-popup-box ty-dropdown-box__content {if $runtime.controller != 'index'}hidden{/if}">
