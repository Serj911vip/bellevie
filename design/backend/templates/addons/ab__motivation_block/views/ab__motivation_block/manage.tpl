...
<form action="{""|fn_url}" method="post" name="ab__mb_motivation_items_form" enctype="multipart/form-data">
...
<th width="60%"><a class="cm-ajax" href="{"`$c_url`&sort_by=name&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("name")}{if $search.sort_by == "name"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
...
<td>
<a class="row-status" href="{"ab__motivation_block.update?motivation_item_id=`$motivation_item.motivation_item_id`"|fn_url}">{$motivation_item.name}</a></td>
...
{include file="common/select_popup.tpl" id=$motivation_item.motivation_item_id status=$motivation_item.status hidden=true object_id_name="motivation_item_id" table="ab__mb_motivation_items" popup_additional_class="cm-no-hide-input dropleft"}
</td>
</tr>
{/foreach}
</table>
